...
@php
   ...
   if($company){
   $companyAddress = DB::table('companies_Address')->where('id', $company->address_id)->first();}else{
...
">
  ...
  @endif
            ...
            @if($company)
                ...
                @else
                <h2>{{ $user->name }} <span class="badge badge-info">{{ $user->role->name }}</span></h2>
                <p>{{ $user->email }}</p>
...
@if($company)
                ...
                <p>
                    @if ($user)
                        ...
                        <strong>Address:</strong> {{  $companyAddress->address_complete }}
                    @else
            ...
            @else
                ...
                <p>
                    @if ($user)
                        <strong>Phone:</strong> {{ $user->Phone }}<br>
                        <strong>Bio:</strong> {{ $medicalTeam->medicalservice->professional_bio }}<br>
                        <strong>Address:</strong> {{  $medicaladdress->address_complete }}
                    @else
                        No additional details available.
                    @endif
                </p>
